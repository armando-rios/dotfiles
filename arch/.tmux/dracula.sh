...
main()
{
  ...
  # Redefine las variables principales
  white=$text
  gray=$surface1
  dark_gray=$base
  light_purple=$mauve
  dark_purple=$surface0
  cyan=$sky
  green=$green
  orange=$peach
  pink=$pink
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${blue},fg=${dark_gray}]#{?client_prefix,#[bg=${green}],} ${left_icon} #H #[fg=${blue},bg=${bg_color}]#{?client_prefix,#[fg=${green}],}${left_sep}"
    powerbg=${bg_color}
  else
    tmux set-option -g status-left "#[bg=${green},fg=${dark_gray}]#{?client_prefix,#[bg=${green}],} ${left_icon}"
  fi
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if case $plugin in custom:*) true;; *) false;; esac; then
      ...
      IFS=' ' read -r -a colors  <<< $(get_tmux_option "@dracula-cwd-colors" "pink dark_gray")
      ...
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-time-colors" "dark_purple pink")
  ...
  # Window option
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[fg=${window_sep_fg},bg=${mauve}]${window_sep}#[fg=${dark_purple},bg=${mauve}]   #W${current_flags} #[fg=${mauve},bg=${bg_color}]${left_sep}"
  else
    tmux set-window-option -g window-status-current-format "#[fg=${white},bg=${dark_purple}] #I #W${current_flags} "
  fi

  tmux set-window-option -g window-status-format "#[fg=${window_sep_fg},bg=${surface1}]${window_sep}#[fg=${white}]#[bg=${surface1}]   #W${flags} #[fg=${surface1},bg=${bg_color}]${left_sep}"
